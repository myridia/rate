...
pub async fn ecb() -> Result<String, Box<dyn Error>> {
    ...
    //let p: Envelope = from_str(&res).unwrap();
    //println!("{:?}", p);
...
pub async fn ecb(Query(params): Query<HashMap<String, String>>) -> impl IntoResponse {
    let mut r = Rated {
